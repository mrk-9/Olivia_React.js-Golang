	...
	<div class="page regular" @click="contextMenuVisibility = false">
		<div class="header-wrap">
			<div class="header">
				...
				<div class="menu" v-if="this.user.id != 0">
					<router-link v-for="(title, link) in this.menuItems" :class="{ item: true, active: (currentRoute == link) }"
						 :to="link">{{ title }}</router-link>
					<div :class="{ item: true, active: (currentRoute == '/upgrade') }">
						<span class="big-title">Free Trial</span>
						<span class="small-title"> - 10 days remaining</span>
...
<script>
  ...
  import AuthResource from './../../resources/Auth'
	...
	import intercom from './../vendor/intercom'
    ...
    data () {
      ...
      AuthResource.getProfile().then(
        response => {
          ...
          intercom(this.user.fname + ' ' + this.user.sname, this.user.email, this.user.created_at, true)
        },
				response => {
          intercom()
        }
